...
const enum_converters_1 = require("./lib/enum-converters");
(() => {
    document.addEventListener("DOMContentLoaded", () => {
        const selector = document.getElementById('resource-selector');
        const displayer = document.getElementById('resource-displayer');
        ...
        selector.addEventListener("change", () => {
            ...
            fetch(`/api/resources?ResourceType=${newValue}&IncludeMod=true&IncludeTags=true&OrderBy1=1&Asc=true`).then(response => response.json()).then((response) => {
                response.map(r => {
